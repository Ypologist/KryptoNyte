...
validate_configuration() {
    ...
    if ! riscof validateconfig --config="$CONFIG_FILE" >/dev/null 2>&1; then
        print_error "Invalid RISCOF configuration file: $CONFIG_FILE"
        print_step "Running configuration validation for details:"
        riscof validateconfig --config="$CONFIG_FILE"
        exit 1
    fi
