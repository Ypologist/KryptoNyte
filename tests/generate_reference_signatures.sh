...
generate_reference() {
    ...
    if ! run_with_timeout 60 "riscv_isac signature generation" \
        riscv_isac --test-signature-output "$test_dir/${test_name}.signature" \
                  --no-compliance \
                  --isa rv32i \
                  --signature-granularity 4 \
                  --signature-format hex \
                  --test "$elf_file" \
                  --work-dir "$isac_dir" > "$log_file" 2>&1; then
        echo "❌ riscv_isac signature generation failed for $test_name"
        echo "  Check log file: $log_file"
        return 1
    fi
    
    # Signature file should already be generated by riscv_isac above
