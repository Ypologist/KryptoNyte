...
usage() {
  cat <<USAGE
Usage: $(basename "$0") --kernel <name> [--out-dir <path>] [--max-cycles <count>] [--keep-output] [--force-sim-build]

Builds and runs tests/functional_tests/<name>.c on:
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --kernel)
      if [[ $# -lt 2 ]]; then
        echo "Error: --kernel requires a name" >&2
        exit 1
      fi
      KERNEL="$2"
      shift 2
...
KERNEL_SRC="$SCRIPT_DIR/${KERNEL}.c"
...
if [[ -z "$OUT_DIR" ]]; then
  OUT_DIR="$REPO_ROOT/tests/output/functional_tests/${KERNEL}/zeronyte"
fi
...
echo "[1/4] Building RV32 test ELF for kernel '$KERNEL'..."
"$RISCV_GCC" \
  -march="$ISA" \
  -mabi=ilp32 \
  -mcmodel=medany \
...
if [[ "$FORCE_SIM_BUILD" -eq 1 || ! -x "$ZERONYTE_SIM" ]]; then
  ...
  echo "[3/4] Reusing existing ZeroNyte RTL simulator..."
fi
