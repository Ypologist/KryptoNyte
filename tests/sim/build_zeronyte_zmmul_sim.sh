...
OBJ_DIR="$BUILD_DIR/zeronyte_zmmul_obj"
...
verilator -cc "$VERILOG_TOP" \
  --top-module ZeroNyteRV32IZmmulCore \
  --Mdir "$OBJ_DIR" \
  ...
  -LDFLAGS "-O2" \
  --exe \
    "$SIM_DIR/zeronyte_zmmul_sim.cpp" \
    "$SIM_DIR/elf_loader.cpp" \
    "$SIM_DIR/memory.cpp"

cp "$OBJ_DIR/VZeroNyteRV32IZmmulCore" "$BUILD_DIR/zeronyte_zmmul_sim"
chmod +x "$BUILD_DIR/zeronyte_zmmul_sim"

echo "Built simulator at $BUILD_DIR/zeronyte_zmmul_sim"
