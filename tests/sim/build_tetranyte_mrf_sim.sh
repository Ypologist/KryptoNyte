...
OBJ_DIR="$BUILD_DIR/tetranyte_mrf_obj"

mkdir -p "$BUILD_DIR"
rm -rf "$OBJ_DIR"
mkdir -p "$OBJ_DIR"

VERILOG_TOP="rtl/generators/generated/verilog_hierarchical_timed/TetraNyteRV32ICoreMRF.v"
...
verilator -cc "$VERILOG_TOP" \
  --top-module TetraNyteRV32ICoreMRF \
  --Mdir "$OBJ_DIR" \
  ...
  -LDFLAGS "-O2" \
  --exe \
    "$SIM_DIR/tetranyte_mrf_sim.cpp" \
    "$SIM_DIR/elf_loader.cpp" \
    "$SIM_DIR/memory.cpp"

cp "$OBJ_DIR/VTetraNyteRV32ICoreMRF" "$BUILD_DIR/tetranyte_mrf_sim"
chmod +x "$BUILD_DIR/tetranyte_mrf_sim"

echo "Built simulator at $BUILD_DIR/tetranyte_mrf_sim"
