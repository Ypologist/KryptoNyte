...
# Place in: KryptoNyte/physical_design/
# Run from: KryptoNyte/physical_design/ with ./generate_physical_design.sh
...
TOP_MODULE=""
INPUT_RTL_PATH="../rtl/generators/generated/verilog_hierarchical_timed"
...
Options:
  ...
  --input-rtl <path>          Input RTL directory (default: ../rtl/generators/generated/verilog_hierarchical_timed)
...
Examples:
  # Complete flow for ZeroNyte core (run from physical_design/ directory)
  ./generate_physical_design.sh --module-name ZeroNyteRV32ICore
  
  # Synthesis only with custom clock
  ./generate_physical_design.sh --module-name ALU32 --clock-period 5.0 --synthesis-only
  
  # Custom OpenLane2 path and output location
  ./generate_physical_design.sh --openlane2-path /opt/skywater-pdk/openlane2 --output-root /tmp/physical
