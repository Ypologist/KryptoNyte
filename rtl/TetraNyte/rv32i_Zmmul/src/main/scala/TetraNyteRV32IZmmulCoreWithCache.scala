...
class TetraNyteRV32IZmmulCoreWithCacheIO(val numThreads: Int, val cosimulate: Boolean = false) extends Bundle {
...
class TetraNyteRV32IZmmulCoreWithCache(val cosimulate: Boolean = false) extends Module {
  val numThreads = 4
  val io = IO(new TetraNyteRV32IZmmulCoreWithCacheIO(numThreads, cosimulate))
  io.jtag_tdo := 0.U
