...
class TetraNyteRV32IMCoreIO(val numThreads: Int, val cosimulate: Boolean = false) extends Bundle {
...
class TetraNyteRV32IMCore(val cosimulate: Boolean = false) extends Module {
  val numThreads = 4
  val io = IO(new TetraNyteRV32IMCoreIO(numThreads, cosimulate))
  io.jtag_tdo := 0.U
