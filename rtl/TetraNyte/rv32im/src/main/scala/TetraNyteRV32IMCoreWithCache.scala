...
class TetraNyteRV32IMCoreWithCacheIO(val numThreads: Int, val cosimulate: Boolean = false) extends Bundle {
...
class TetraNyteRV32IMCoreWithCache(val cosimulate: Boolean = false) extends Module {
  val numThreads = 4
  val io = IO(new TetraNyteRV32IMCoreWithCacheIO(numThreads, cosimulate))
  io.jtag_tdo := 0.U
